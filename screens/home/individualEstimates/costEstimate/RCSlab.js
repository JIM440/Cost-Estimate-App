...
const RCSlab = () => {
    ...
    const numRodsX = Math.ceil(parseInt(RCSLabLength) / parseInt(RCSLabRodSpacing));
    const num12mRodsX = Math.ceil(numRodsX * parseInt(RCSLabWidth) / 12);

    const numRodsY = Math.ceil(parseInt(RCSLabWidth) / parseInt(RCSLabRodSpacing));
    ...
    return (
        <ScrollView style={containerStyles.scrollContainer}>
          <Image style={image.image} source={require('../../../../assets/images/individual_estiamte/rc_slab_c.jpg')} />
            <View style={containerStyles.container}>
                ...
                <View style={inputStyles.threeColumn}>
                    <TextInputTitle
                        ...
                        value={RCSLabLength}
                        title='Length (m)' onChange={(text) => setRCSLabLength(text)}
                    />
                    <TextInputTitle
                        style={inputStyles.threeColumnInput}
                        placeholder="Enter Width"
                        value={RCSLabWidth}
                        title='Width (m)' onChange={(text) => setRCSLabWidth(text)}
                    />
                    <TextInputTitle
                        style={inputStyles.threeColumnInput}
                        placeholder="Enter Thickness"
                        value={RCSLabHeight}
                        title='Thickness (m)' onChange={(text) => setRCSLabHeight(text)}
                    />
                </View>
                    <TextInputTitle
                        placeholder="Enter Spacing"
                        value={RCSLabRodSpacing}
                        title='Spacing (m)' onChange={(text) => setRCSLabRodSpacing(text)}
                    />
      ...
      <View style={tableStyles.row}>
        ...
        <View style={tableStyles.column}>
          <Text style={tableStyles.cell}>{RCSlabEstimate.numBagsCement}</Text>
        </View>
        <View style={tableStyles.column}>
